...
// Estado del índice para cada slider
let currentIndex1 = 0;
let currentIndex2 = 0;
const imagesPerSlide = 4;  // Mostrar 4 imágenes por slide

function moveSlide(sliderNumber, direction) {
  let sliderWrapper, currentIndex;

  // Identificar el slider en función de su número
  if (sliderNumber === 1) {
    sliderWrapper = document.getElementById('sliderWrapper1');
    currentIndex = currentIndex1;
  } else if (sliderNumber === 2) {
    sliderWrapper = document.getElementById('sliderWrapper2');
  ...
  // Actualizar el índice actual en función de la dirección
  currentIndex = (currentIndex + direction + totalSlides) % totalSlides;

  // Calcular el desplazamiento en función del tamaño de la imagen
  const imageWidth = sliderWrapper.children[0].offsetWidth;
  const translateXValue = -currentIndex * (imageWidth * imagesPerSlide); 
